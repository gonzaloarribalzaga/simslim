...
// tvOSCategories mirrors the iOS mutation catalog. A full 170-label profile
// was applied to a disposable tvOS 26.4 simulator, persisted through reboot,
// and still booted TV Settings and Preview Shell. Memory estimates remain iOS
// measurements and therefore are deliberately omitted on tvOS.
var tvOSCategories = tvOSCatalogFromIOS()

func tvOSCatalogFromIOS() []Category {
	catalog := make([]Category, len(Categories))
	for i, category := range Categories {
		category.ApproxMemoryMB = 0
		category.Labels = append([]string(nil), category.Labels...)
		category.AlwaysEnabled = append([]AlwaysEnabledService(nil), category.AlwaysEnabled...)
		category.ServiceDescriptions = nil
		catalog[i] = category
	}
	return catalog
}
